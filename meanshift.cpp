...
int main( int argc, const char** argv )
{
 ...
 //   int camNum = parser.get<int>("1");     
    ...
    cap.open("00000.avi");//直接调用成员函数打开摄像头
/*
    if( !cap.isOpened() )
    {
        help();
...
*/
    ...
    setMouseCallback( "MeanShift Demo", onMouse, 0 );//消息响应机制
    createTrackbar( "Vmin", "MeanShift Demo", &vmin, 256, 0 );//createTrackbar函数的功能是在对应的窗口创建滑动条，滑动条Vmin,vmin表示滑动条的值，最大为256
    createTrackbar( "Vmax", "MeanShift Demo", &vmax, 256, 0 );//最后一个参数为0代表没有调用滑动拖动的响应函数
    createTrackbar( "Smin", "MeanShift Demo", &smin, 256, 0 );//vmin,vmax,smin初始值分别为10,256,30
